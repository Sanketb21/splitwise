...
import com.splitwise.expenseservice.model.UserBalance;
...
public class ExpenseServiceImpl implements ExpenseService {
    ...
    public ExpenseResponse createExpense(ExpenseRequest expenseRequest, Long createdBy) {
        ...
        Expense expense = Expense.builder()
                ...
                .expenseDate(expenseRequest.getExpenseDate() != null ? expenseRequest.getExpenseDate()
    ...
    @Override
    @Transactional(readOnly = true)
    public List<SimplifiedDebtResponse> getSimplifiedDebtsByGroup(Long groupId) {
        log.info("Calculating simplified debts for group ID: {}", groupId);
        ...
        if (expenses.isEmpty()) {
            log.info("No expenses found for group ID: {}", groupId);
            return new ArrayList<>();
        }

        // Calculate net balance for each user
        // Net balance = (total paid by user) - (total owed by user)
        // Positive balance = user should receive money
        // Negative balance = user should pay money
        Map<Long, BigDecimal> netBalances = calculateNetBalances(groupId, expenses);
        ...
        List<SimplifiedDebtResponse> simplifiedDebts = minimizeTransactions(creditors, debtors, groupId);

        log.info("Simplified {} transactions for group ID: {}", simplifiedDebts.size(), groupId);
     ...
     */
    private Map<Long, BigDecimal> calculateNetBalances(Long groupId, List<Expense> expenses) {
        Map<Long, BigDecimal> netBalances = new java.util.HashMap<>();
        ...
        for (Expense expense : expenses) {
            ...
            netBalances.put(paidBy, netBalances.getOrDefault(paidBy, BigDecimal.ZERO).add(totalAmount));

            // Subtract the owed amounts from participants
            List<ExpenseParticipant> participants = expenseParticipantRepository.findByExpenseId(expense.getId());
            for (ExpenseParticipant participant : participants) {
                Long userId = participant.getUserId();
                BigDecimal amountOwed = participant.getAmountOwed();

                netBalances.put(userId, netBalances.getOrDefault(userId, BigDecimal.ZERO).subtract(amountOwed));
            }
        }
     ...
     * Match the largest debtor with the largest creditor iteratively
     */
    private List<SimplifiedDebtResponse> minimizeTransactions(
            List<UserBalance> creditors, List<UserBalance> debtors, Long groupId) {
        ...
        while (i < creditors.size() && j < debtors.size()) {
            ...
            // Determine the settlement amount (min of what creditor should receive and
            // debtor should pay)
            ...
            // Update balances
            creditor.setAmount(creditor.getAmount().subtract(settlementAmount));
            debtor.setAmount(debtor.getAmount().subtract(settlementAmount));
